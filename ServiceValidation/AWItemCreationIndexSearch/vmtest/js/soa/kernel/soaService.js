...
define( [
...
], function( app, superagent, _, assert, debug, logger, eventBus, browserUtils, localStrg ) {
    'use strict';
     ...
     */
    var defaultJSessionIDSOA = 'JSESSION=LOGGED_OFF';

    /**
     * ID of the current session.
     */
    var _JSessionIDSOA = defaultJSessionIDSOA;
            ...
            var jsonData = {
                header: {
                    state: {
                         ...
                         */
                        logCorrelationID: logger.getCorrelationID().replace( /[^\x20-\x7E]+/g, '' ),
                        /**
...
//logger.info(JSON.stringify(jsonData));
//logger.info(browserUtils.getBaseURL() + 'tc/JsonRestServices/' + endPt + JSON.stringify(jsonData, { headers: headers }));
...
///		const call = post( `${state.url}/tc/JsonRestServices/${soaApi}` );
...
//			return $http.post( 'http://10.134.64.101:3000' + '/tc/JsonRestServices/' + endPt, jsonData, {
//                headers: headers
//            } ).then( function( response ) {
			
			//if( jsonData.cookie ) { call.set( 'Cookie', state.cookie ); }
			
			
			//if ( operationName == 'login' ) { jsonData.cookie = _JSessionIDSOA }
			
			var setcookie = true;
			
			if (_JSessionIDSOA !== defaultJSessionIDSOA) { var setcookie = false }
			
			return superagent.post(process.env.AW_PROXY_SERVER + '/tc/JsonRestServices/' + endPt)
			.set( 'Cookie', _JSessionIDSOA )
			.send( jsonData )
			.then( function( response ) {
...
//				var response = unescape(response);
				...
				if (setcookie) {
//					_.forEach( response.header[ 'set-cookie' ], function( cookie ) {
//						if( cookie && cookie.trim().indexOf( 'JSESSIONID=' ) === 0 ) {
//							var _JSessionIDSOA = cookie.trim();
//							logger.info('COOKIE COOKIE: ' + _JSessionIDSOA)
//						}
//					} );

//					jsonData.cookie = response.header[ 'set-cookie' ];
					_JSessionIDSOA = response.header[ 'set-cookie' ];
					process.env.JSessionID = _JSessionIDSOA;
				}
			...
			//logger.info('TESTESTESTESTTS');
			//logger.info( 'endPt=' + endPt, response );
    ...
    exports.post = function( serviceName, operationName, body, propertyPolicyOverride ) {
        return exports.postUnchecked( serviceName, operationName, body, propertyPolicyOverride ).then( function( response ) {
			...
			//process.env.JSessionID = response.cookie;
			//logger.info(JSON.stringify(response));
			
			//var _JSessionIDSOA = response.header[ 'set-cookie' ];
			//process.env.JSessionID = _JSessionIDSOA;
